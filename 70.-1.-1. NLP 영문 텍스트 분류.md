...
```python
qmark = np.mean(train_df['review'].apply(lambda x: x[-2]=='?'))
qmark2 = np.mean(train_df['review'].apply(lambda x: '?' in x))
period = np.mean(train_df['review'].apply(lambda x: x[-2]=='.'))
first_capital = np.mean(train_df['review'].apply(lambda x: x[1].isupper()))
